...
do
  videos+=("$line")
done < <(echo "$webpage" | iconv -f 'ISO-8859-1' -t "UTF-8" | grep -o "ChangeMessage('[^']*', 'menu')" | sed "s/ChangeMessage('//;s/', 'menu')//" | grep -v "^&nbsp;$")

ids=()
while read -r line
do
  ids+=("$line")
done < <(echo "$webpage" | iconv -f 'ISO-8859-1' -t "UTF-8" | grep -Eo 'href="https://medias.lenodal.com/video.php\?id=[0-9]*"' | grep -Eo 'id=[0-9]*' | sed 's/id=//')

imgs=()
while read -r line
do
  imgs+=("$line")
done < <(echo "$webpage" | iconv -f 'ISO-8859-1' -t "UTF-8" | grep -o 'src="https://medias.lenodal.com/medias/photos/[^"]*"' | sed 's/src="//;s/"//')

IMG="${imgs[0]}"
base64_img="$(curl -s "$IMG" --output - | base64)"
